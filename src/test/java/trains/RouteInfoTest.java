...
public class RouteInfoTest {
    ...
    @Test
    public void testNullShortestRouteLength() {
        Integer result = RouteInfo.shortestRouteLength(testGraph, B, A);
        assertNull("Incorrect result for nonexistent shortest route A-C", result);
    }

    @Test
    public void testMaxPathLength() {
        int result = RouteInfo.boundedPaths(testGraph, C, C, 1, 2, true);
        // CDC is only route length 2
        assertEquals("Incorrect result for no of routes C-C max length 2", 1, result);
    }

    @Test
    public void testMinPathLength() {
        int result = RouteInfo.boundedPaths(testGraph, C, C, 3, 4, true);
        // CDCD, CDEBC, CEBC are the valid routes
        assertEquals("Incorrect result for no of routes C-C min length 3 max length 4", 3, result);
    }

    @Test
    public void testMaxPathWeight() {
        int result = RouteInfo.boundedPaths(testGraph, C, C, 1, 27, false);
        assertEquals("Incorrect result for no of routes C-C max weight 27", 7, result);
    }

    @Test public void testZeroPathWeightCount() {
        int result = RouteInfo.boundedPaths(testGraph, B, A, 1, 30, true);
        assertEquals("Incorrect result for no of routes B-A", 0, result);
    }
}
